...
```{r}
# load data
kr = read.csv("data_fig1_kimrep.csv")
...
write.csv(learningCurveDF,"output_fig1PanelA_binnedLC_meanSEM.csv", row.names = FALSE)
```
...
write.csv(early_lr_smry,"output_fig1PanelB_earlyLR_meanSEM.csv", row.names = TRUE)
```
...
write.csv(asym_smry,"output_fig1PanelC_asym1_meanSEM.csv", row.names = TRUE)
```
...
write.csv(wash_smry,"output_fig1PanelD_wash_meanSEM.csv", row.names = TRUE)
```
