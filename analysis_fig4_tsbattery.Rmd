...
pairedTAndCohensD <- function(dataA, dataB) {
  ...
  testout = effsize::cohen.d(dataA, dataB, paired = TRUE)
  d = testout$estimate
  return(c(t, df, p, d))
}
...
```{r}
# load data
kr = read.csv("data_fig4_TargetSizeTargetJump.csv")
...
write.csv(lrn_smry,"output_fig4PanelA_STLXClamp_meanSEM.csv", row.names = TRUE)
```
...
write.csv(success_smry,"output_fig4PanelB_STLXSuccess_meanSEM.csv", row.names = TRUE)
...
write.csv(successClamp_smry,"output_fig4PanelC_STLXSuccessClamp_meanSEM.csv", row.names = TRUE)
...
write.csv(effsizeForPlot,"output_fig4PanelD_effsizeXSuccessClamp_mean.csv", row.names = TRUE)
```
